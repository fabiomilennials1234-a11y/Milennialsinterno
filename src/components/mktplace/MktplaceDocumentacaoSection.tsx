...
import { useMktplaceDocumentation } from '@/hooks/useMktplaceKanban';
...
export default function MktplaceDocumentacaoSection() {
  const { data: docs = [], isLoading } = useMktplaceDocumentation();
  ...
  docs.forEach((doc: any) => {
    ...
    docsByDate[dateKey].entries.push({
      ...
      clientName,
      tracking_type: doc.tracking_type,
                    ...
                    <div key={entry.id} className="p-2.5 bg-muted/30 rounded-lg space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant="outline" className={cn(
                          "text-[9px]",
                          entry.tracking_type === 'gestao'
                            ? 'bg-purple-500/10 text-purple-500 border-purple-500/30'
                            : 'bg-blue-500/10 text-blue-500 border-blue-500/30'
                        )}>
                          {entry.tracking_type === 'gestao' ? 'Gestão' : 'Consultoria'}
                        </Badge>
                      </div>

                      {/* Falou com cliente (só gestão) */}
                      {entry.tracking_type === 'gestao' && entry.falou_com_cliente && (
