...
interface CreateJustificationVars {
  cardId: string;
  justification: string;
}

// Resultado de useMutation; tipo amplo para aceitar qualquer hook do factory.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type CreateJustificationMutation = UseMutationResult<any, Error, CreateJustificationVars, unknown>;

interface KanbanDelayModalProps {
  isOpen: boolean;
  onClose: () => void;
  delayedCards: DelayedCard[];
  /** Hook que retorna useMutation pra criar justificativa (delegue ao factory). */
  useCreateJustification: () => CreateJustificationMutation;
  /** Título do header do modal. Default: "Demanda Atrasada". */
  title?: string;
}

/**
 * Modal genérico de justificativa de atraso para boards especializados.
 * Substitui DesignDelayModal, VideoDelayModal, DevsDelayModal,
 * ProdutoraDelayModal — todos com lógica idêntica.
 */
export default function KanbanDelayModal({
  isOpen,
  onClose,
  delayedCards,
  useCreateJustification,
  title = 'Demanda Atrasada',
}: KanbanDelayModalProps) {
