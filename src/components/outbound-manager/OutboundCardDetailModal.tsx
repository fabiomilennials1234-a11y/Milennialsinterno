...
import ProToolContentView from '../ads-manager/ProToolContentView';
import OutboundBonusTableView from './OutboundBonusTableView';
...
interface OutboundCardDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  card: CardData | null;
...
const AVAILABLE_TAGS = [
  ...
  { label: 'Concluido', color: 'bg-success/80' },
  { label: 'Pendente', color: 'bg-info/80' },
  { label: 'Prioridade', color: 'bg-primary' },
];

export default function OutboundCardDetailModal({
  isOpen,
  onClose,
  card,
  onSave,
              ...
              >
                {description ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    {readOnly ? 'Sem conteudo disponivel' : 'Adicione uma descricao mais detalhada...'}
                  </p>
                )}
              </div>
            )}
