...
import {
  useClientVideoProfiles,
  type ClientWithVideoProfile,
} from '@/hooks/useClientVideoProfiles';
...
interface ClientCardProps {
  client: ClientWithVideoProfile;
  onEdit: (client: ClientWithVideoProfile) => void;
}
      ...
      {hasProfile && (
        <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-border/50">
          <ProfileField
            ...
            label="Estilo de edicao"
            value={profile.editing_style}
          />
          <ProfileField
            icon={<Film size={12} />}
            label="Formatos"
          ...
          <ProfileField
            ...
            label="Intro/Outro"
            value={profile.intro_outro_url}
            isLink
          />
          <ProfileField
            icon={<LinkIcon size={12} />}
            label="Referencias"
            value={profile.reference_urls}
            isLink
          />
          <ProfileField
            icon={<LinkIcon size={12} />}
            label="Assets da marca"
            value={profile.brand_assets_url}
            isLink
          />
          <ProfileField
...
export default function VideoClientInfoTab() {
  const { data: clients, isLoading } = useClientVideoProfiles();
  const [search, setSearch] = useState('');
  const [selectedClient, setSelectedClient] = useState<ClientWithVideoProfile | null>(null);
  ...
  const handleEdit = (client: ClientWithVideoProfile) => {
