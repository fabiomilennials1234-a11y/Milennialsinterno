...
import { ClipboardList, Plus, Edit, Trash2, Loader2, ExternalLink, Copy, Check, Globe, GlobeLock } from 'lucide-react';
...
import {
  useClientPaddockDiagnosticos,
  useDeletePaddockDiagnostico,
  usePublishPaddockDiagnostico,
  calcPaddockAreaScores,
  calcPaddockNotaGeral,
  calcPaddockPrioridade,
  type PaddockDiagnosticoRecord,
} from '@/hooks/usePaddockDiagnostico';
import PaddockDiagnosticoFormModal from './PaddockDiagnosticoFormModal';
...
export default function PaddockDiagnosticoListSection({ clientId, clientName }: Props) {
  const { data: diagnosticos, isLoading } = useClientPaddockDiagnosticos(clientId);
  const deleteDiag = useDeletePaddockDiagnostico();
  const publishDiag = usePublishPaddockDiagnostico();
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editing, setEditing] = useState<PaddockDiagnosticoRecord | undefined>(undefined);
  const [deleteConfirmId, setDeleteConfirmId] = useState<string | null>(null);

  const handleEdit = (d: PaddockDiagnosticoRecord) => {
    setEditing(d);
    setIsFormOpen(true);
  };
  ...
  const getDiagUrl = (d: PaddockDiagnosticoRecord) => `${window.location.origin}/diagnostico-paddock/${d.public_token}`;

  const handleCopyLink = async (d: PaddockDiagnosticoRecord) => {
    try {
  ...
  const handleTogglePublish = async (d: PaddockDiagnosticoRecord) => {
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {diagnosticos.map((diag) => {
              const areas = calcPaddockAreaScores(diag);
              const nota = calcPaddockNotaGeral(areas);
              const prioridade = calcPaddockPrioridade(nota);
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {diag.data_consultoria
                        ...
                        : diag.created_at
                        ? format(new Date(diag.created_at), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })
                        : 'Data não informada'}
