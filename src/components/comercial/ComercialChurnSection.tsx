...
import { useComercialChurnClients } from '@/hooks/useComercialClients';
import { useAuth } from '@/contexts/AuthContext';
import { UserX, Archive, AlertTriangle, Timer, RotateCcw } from 'lucide-react';
...
export default function ComercialChurnSection() {
  const { data: clients = [], isLoading } = useComercialChurnClients();
  ...
  // Check if user can archive (CEO, gestor_projetos, consultor_comercial, sucesso_cliente)
  const canArchive = isCEO || ['gestor_projetos', 'consultor_comercial', 'sucesso_cliente'].includes(user?.role || '');
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <ClientLabelBadge
                            label={((client as any)?.client_label ?? null) as ClientLabel}
                            size="sm"
                            className="shrink-0"
                          />
                        </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <div className="flex items-center gap-2 min-w-0">
                            <h4 className="font-semibold text-sm text-muted-foreground truncate">{client.name}</h4>
                            <ClientLabelBadge
                              label={((client as any)?.client_label ?? null) as ClientLabel}
                              size="sm"
                              className="shrink-0"
