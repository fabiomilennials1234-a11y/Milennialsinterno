...
interface CreateProdutoraCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: any) => void;
  isLoading?: boolean;
  produtoraColumns: { id: string; title: string }[];
}

export default function CreateProdutoraCardModal({
  isOpen,
  onClose,
  onSubmit,
  isLoading = false,
  produtoraColumns,
}: CreateProdutoraCardModalProps) {
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Gravação vídeo institucional cliente X"
            ...
            <div>
              ...
              <select
                ...
                required
              >
                <option value="">Selecione a produtora...</option>
                {produtoraColumns.map(col => (
            ...
            <div>
              ...
              <textarea
                value={formData.briefing.observations}
                onChange={e => setFormData(prev => ({ 
                  ...prev, 
                  briefing: { ...prev.briefing, observations: e.target.value }
                }))}
                placeholder="Detalhes importantes sobre a gravação..."
                rows={3}
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">
                Vídeo de Referência (Mínimo 1) *
              </label>
              <input
                type="url"
                value={formData.briefing.reference_video_url}
                onChange={e => setFormData(prev => ({ 
                  ...prev, 
                  briefing: { ...prev.briefing, reference_video_url: e.target.value }
                }))}
                placeholder="https://youtube.com/... ou link do Drive"
                className="input-apple"
              />
            </div>
          </div>
