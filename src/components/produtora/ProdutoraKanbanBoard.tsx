...
export default function ProdutoraKanbanBoard() {
  ...
  const cardsByColumnAndStatus = useMemo(() => {
    const statusIds = new Set(PRODUTORA_STATUSES.map(s => s.id));
    const buckets: Record<string, Record<string, KanbanCard[]>> = {};
    for (const card of cards) {
      if (!buckets[card.column_id]) {
        buckets[card.column_id] = {};
        for (const s of PRODUTORA_STATUSES) buckets[card.column_id][s.id] = [];
      }
      const status = card.status && statusIds.has(card.status) ? card.status : 'a_gravar';
  ...
  const getCardsForColumn = (columnId: string) => {
    return cardsByColumnAndStatus[columnId] || PRODUTORA_STATUSES.reduce((acc, s) => {
      acc[s.id] = [];
      return acc;
