...
// V1 types come from GROWTH_TASK_PREFIX keys.
// V2 types are the step-based descriptions generated by growth_advance_gp_step RPC.
type GrowthTaskTypeV1 = keyof typeof GROWTH_TASK_PREFIX;
type GrowthTaskTypeV2 =
  | 'marcar_call_1'
  | 'realizar_call_1'
  | 'escolher_equipe'
  | 'alinhar_projeto'
  | 'brifar_crm'
  | 'brifar_crm_alinhar';
type GrowthTaskType = GrowthTaskTypeV1 | GrowthTaskTypeV2;

const GROWTH_TYPES = new Set<string>([
  // V1
  'welcome', 'schedule_call', 'do_call', 'align_project',
  // V2
  'marcar_call_1', 'realizar_call_1', 'escolher_equipe',
  'alinhar_projeto', 'brifar_crm', 'brifar_crm_alinhar',
]);
...
function detectGrowthTaskType(task: { description?: string | null; title: string }): GrowthTaskType | null {
  ...
  // Fallback: title prefix matching (V1 only — V2 always has description)
