...
export function useVideoCompletionNotifications() {
  const { user } = useAuth();

  return useQuery({
    queryKey: ['video-completion-notifications', user?.id],
    queryFn: async (): Promise<VideoCompletionNotification[]> => {
      if (!user?.id) return [];
      ...
      return (data || []) as VideoCompletionNotification[];
    },
    enabled: !!user?.id,
...
export function useMarkVideoNotificationRead() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['video-completion-notifications', user?.id] });
    },
  });
}

// Create completion notification
export function useCreateVideoCompletionNotification() {
  const queryClient = useQueryClient();
  const { user } = useAuth();
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const completedByName = profile?.name || 'Editor';
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['video-completion-notifications'] });
    },
  });
}

// Hook to show toast notifications for completed video requests
export function useVideoCompletionToasts() {
  const { data: notifications = [] } = useVideoCompletionNotifications();
  ...
  useEffect(() => {
    notifications.forEach((notification) => {
      ...
      toast.success(
        `Sua demanda de vídeo "${notification.card_title}" está pronta!`,
        {
          duration: 10000,
...
// Fetch creator info for a card (from video_briefings or kanban_cards)
export function useVideoCardCreatorInfo(cardId: string | undefined) {
  return useQuery({
    queryKey: ['video-card-creator', cardId],
    queryFn: async () => {
      if (!cardId) return null;

      // First try to get from video_briefings
...
export function useMultipleVideoCardsCreators(cardIds: string[]) {
  return useQuery({
    queryKey: ['multiple-video-cards-creators', cardIds.sort().join(',')],
