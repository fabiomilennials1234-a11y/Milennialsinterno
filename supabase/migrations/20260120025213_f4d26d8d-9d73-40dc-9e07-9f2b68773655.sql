...
-- Map day of week to Portuguese day name
...
DECLARE
  day_num INT;
BEGIN
  -- EXTRACT(DOW FROM NOW()) returns 0=Sunday, 1=Monday, etc.
  day_num := EXTRACT(DOW FROM NOW());
  ...
  RETURN CASE day_num
    ...
    WHEN 4 THEN 'quinta'
    WHEN 5 THEN 'sexta'
    WHEN 6 THEN 'segunda' -- Saturday -> Monday
    WHEN 0 THEN 'segunda' -- Sunday -> Monday
  END;
END;
$$;

-- Update the trigger to create tracking record when campaign is published
...
BEGIN
  ...
  IF NEW.status = 'done' AND (OLD.status IS NULL OR OLD.status != 'done') THEN
    ...
    CASE NEW.task_type
      ...
      -- MILESTONE 2: Criar e enviar estratégia
    ...
    IF is_advancing_task THEN
      ...
      -- Create auxiliary tasks for Milestone 3 (when enviar_estrategia is completed)
