...
--
-- Hotfix: corrige append de arrays na RPC upsert_kanban_briefing.
...
BEGIN
  ...
  FOREACH v_field IN ARRAY v_allowed_fields LOOP
    IF _payload ? v_field THEN
      v_insert_columns := v_insert_columns || ARRAY[quote_ident(v_field)];
      v_insert_values := v_insert_values || ARRAY[quote_nullable(_payload ->> v_field)];
      v_update_sets := v_update_sets || ARRAY[format('%I = %s', v_field, quote_nullable(_payload ->> v_field))];
    END IF;
  END LOOP;
  ...
  IF v_existing_id IS NULL THEN
    v_insert_columns := v_insert_columns || ARRAY['created_by'];
    v_insert_values := v_insert_values || ARRAY[quote_nullable(v_caller::text)];
