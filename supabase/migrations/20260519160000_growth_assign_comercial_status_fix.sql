-- 20260519160000_growth_assign_comercial_status_fix.sql
--
-- BUG FIX: growth_assign_full_team was not setting comercial_status/comercial_entered_at
-- when assigning a comercial consultant. This made the client invisible in the
-- treinador comercial kanban (useComercialNewClients filters on comercial_status = 'novo').
--
-- Changes:
--   1. Add comercial_status + comercial_entered_at to the UPDATE in the RPC
--   2. Backfill 4 affected production clients
--
-- Affected clients: ZapLub, Freedom Cursos, Bella Italia, Dra. Isabella Rosa
...
-- Backfill: clients with comercial assigned but no comercial_status
UPDATE public.clients
   SET comercial_status = 'novo',
       comercial_entered_at = now(),
       updated_at = now()
 WHERE assigned_comercial IS NOT NULL
   AND comercial_status IS NULL
   AND archived = false;
