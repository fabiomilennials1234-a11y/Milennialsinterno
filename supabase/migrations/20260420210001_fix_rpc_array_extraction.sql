-- 20260420210001_fix_rpc_array_extraction.sql
--
-- HOTFIX: create_client_with_automations quebrava com 22P02
-- "malformed array literal: insert_client" na PRIMEIRA execução real.
--
-- ROOT CAUSE:
--   v_executed text[] := ARRAY[]::text[];
--   ...
--   v_executed := v_executed || 'insert_client';   -- ← BUG
--
-- Em plpgsql assignment, quando o alvo é text[] e o lado direito mistura
-- `text[] || <string literal>`, o parser resolve o operador para
-- `anyarray || anyarray` em vez de `anyarray || anyelement`, tentando
-- parsear 'insert_client' como array literal. Como a string não começa
-- com '{', estoura 22P02. Repro mínimo:
--
--   DO $$ DECLARE a text[] := '{}'; BEGIN a := a || 'x'; END $$;
--   → ERROR 22P02: malformed array literal: "x"
--
-- O bug só dispara no step `insert_client` porque é o PRIMEIRO append
-- após o primeiro INSERT; a mensagem ecoa o nome do step por acaso
-- (qualquer string literal quebraria).
--
-- FIX: trocar `arr || 'literal'` por `array_append(arr, 'literal')`, que
-- não tem a ambiguidade de tipo. Aplicado em todos os 12 appends em
-- v_executed + 1 em v_warnings. Nenhuma outra mudança — `CREATE OR
-- REPLACE`, assinatura idêntica, error codes idênticos, lógica idêntica.
--
-- Nenhum backfill: RPC nunca chegou a completar em produção, não há
-- rows a reconciliar.
...
COMMENT ON FUNCTION public.create_client_with_automations(jsonb, text) IS
  'Atomic client creation RPC. Replaces useCreateClient hook behind feature flag use_rpc_client_creation. Fixed 2026-04-20: replaced ambiguous "text[] || string_literal" with array_append() to avoid 22P02. Error codes: P0001 generic, P0002 invalid payload, P0003 unauthorized, P0004 cnpj duplicate, P0005 other unique, P0006 fk violation.';
