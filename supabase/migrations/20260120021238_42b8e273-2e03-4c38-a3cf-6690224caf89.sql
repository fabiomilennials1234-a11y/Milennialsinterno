...
BEGIN
  ...
  IF NEW.status = 'done' AND (OLD.status IS NULL OR OLD.status != 'done') THEN
    ...
    IF is_advancing_task THEN
      ...
      -- Create auxiliary tasks after brifar_criativos (Milestone 4 auxiliary tasks)
      IF NEW.task_type = 'brifar_criativos' AND ads_manager_id_val IS NOT NULL THEN
        ...
        VALUES 
          ...
          (NEW.client_id, ads_manager_id_val, 'Brifar otimizações pendentes do(a) ' || COALESCE(client_name, ''), 'brifar_otimizacoes_pendentes', 'Brifar as otimizações pendentes do cliente ' || COALESCE(client_name, '') || '.', 'pending', 4, NOW() + INTERVAL '3 days');
      END IF;
    END IF;
