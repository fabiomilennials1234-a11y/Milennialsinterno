-- Fix: Include GPs with NULL group_id as fallback when resolving GP assignments.
-- Bug: Clients in groups without a matching GP got v_gp_ids = NULL → no auto-task created → stuck.
-- Affected RPCs: growth_advance_gp_step, growth_assign_full_team
-- Data fix: Insert missing alinhar_projeto task for Bagel Licitações (client stuck in alinhar_projeto with no task).

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. RE-CREATE growth_advance_gp_step with GP group fallback fix
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- FIX: Include GPs with NULL group_id as fallback (they serve any group)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 2. RE-CREATE growth_assign_full_team with GP group fallback fix
-- ═══════════════════════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION public.growth_assign_full_team(p_client_id uuid, p_ads uuid, p_comercial uuid, p_crm uuid, p_cx uuid, p_mktplace text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF v_version = 2 AND v_step = 'escolher_equipe' THEN
    ...
    SELECT array_agg(p.user_id)
      ...
      JOIN public.user_roles ur ON ur.user_id = p.user_id
     WHERE ur.role = 'gestor_projetos'
       AND (v_group_id IS NULL OR p.group_id = v_group_id OR p.group_id IS NULL);
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. DATA FIX: Create missing alinhar_projeto task for Bagel Licitações
-- ═══════════════════════════════════════════════════════════════════════════
INSERT INTO public.department_tasks (
  user_id, title, description, task_type, status, priority,
  department, related_client_id, due_date
) VALUES (
  '83deb7c9-5405-4022-a844-f2f98e13d3d7',  -- Weder (GP, group_id NULL)
  'Alinhar Projeto + Briefar Equipe Bagel Licitações',
  'growth:alinhar_projeto',
  'daily', 'todo', 'high',
  'gestor_projetos', '35e813c7-b399-45bb-b44c-441c09b8c6d7',
  now() + interval '1 day'
);
