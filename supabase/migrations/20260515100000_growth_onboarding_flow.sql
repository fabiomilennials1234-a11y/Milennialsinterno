-- 20260515100000_growth_onboarding_flow.sql
--
-- Growth onboarding flow: new client creation path for millennials-growth
-- product when assigned_ads_manager IS NULL (no ADS onboarding).
--
-- Context: Growth clients follow a distinct flow:
--   1. Created with growth_onboarding_step = 'novos_clientes' (no onboarding/tracking)
--   2. CX validates -> dismiss BLOQUEADO tag
--   3. GP schedules Call 1 -> step = 'call_1_agendada'
--   4. After Call 1, GP assigns team via growth_assign_team RPC -> step = 'equipe_designada'
--   5. Client transitions to normal ADS onboarding flow
--
-- Artifacts:
--   1. Column: clients.growth_onboarding_step (text, nullable)
--   2. RPC: growth_assign_team (atomic team assignment after Call 1)
--   3. Modify: create_client_with_automations (Growth-without-ads path)
--   4. Trigger: dismiss "BLOQUEADO: ESPERAR LIGACAO CX" on CX validation
--   5. Trigger: dismiss "Esperar Briefing" when GP completes "Alinhar Projeto" task
--
-- Non-Growth path: UNCHANGED. All changes are gated behind
--   v_is_growth AND v_assigned_ads IS NULL checks.

BEGIN;

-- =============================================================================
-- 1. ADD COLUMN clients.growth_onboarding_step
-- =============================================================================

ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS growth_onboarding_step text DEFAULT NULL;

CREATE INDEX IF NOT EXISTS idx_clients_growth_onboarding_step
  ON public.clients (growth_onboarding_step)
  WHERE growth_onboarding_step IS NOT NULL;

COMMENT ON COLUMN public.clients.growth_onboarding_step IS
  'Growth onboarding flow step. Values: novos_clientes, call_1_agendada, equipe_designada, NULL (normal ADS path or completed).';

-- =============================================================================
-- 2. RPC: growth_assign_team
-- =============================================================================

CREATE OR REPLACE FUNCTION public.growth_assign_team(
  p_client_id        uuid,
  p_ads_manager_id   uuid,
  p_comercial_id     uuid DEFAULT NULL,
  p_mktplace_id      uuid DEFAULT NULL,
  p_has_mktplace     boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ── Fetch client + validate state ──
  SELECT id, growth_onboarding_step, group_id
    INTO v_client
    FROM public.clients
  ...
  IF v_client.growth_onboarding_step IS DISTINCT FROM 'call_1_agendada' THEN
    RAISE EXCEPTION 'client not in call_1_agendada step (current: %)',
      COALESCE(v_client.growth_onboarding_step, 'NULL')
      USING ERRCODE = 'P0002';
  END IF;

  -- ── Permission: admin OR gestor_projetos in same group ──
  IF NOT (
    public.is_admin(v_caller)
    OR (
      public.has_role(v_caller, 'gestor_projetos')
      AND v_client.group_id IS NOT NULL
      AND EXISTS (
        SELECT 1 FROM public.profiles
         WHERE user_id = v_caller
           AND group_id = v_client.group_id
      )
    )
  ) THEN
  ...
  -- ── Validate ads_manager_id exists ──
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE user_id = p_ads_manager_id) THEN
    RAISE EXCEPTION 'ads_manager_id not found in profiles' USING ERRCODE = 'P0002';
  END IF;

  -- ── Update client: assign team + advance step ──
  UPDATE public.clients
     SET assigned_ads_manager   = p_ads_manager_id,
         assigned_comercial     = COALESCE(p_comercial_id, assigned_comercial),
         assigned_mktplace      = CASE
                                    WHEN p_has_mktplace AND p_mktplace_id IS NOT NULL
                                    THEN p_mktplace_id::text
                                    ELSE assigned_mktplace
                                  END,
         mktplace_status        = CASE
                                    WHEN p_has_mktplace AND p_mktplace_id IS NOT NULL
                                    THEN COALESCE(mktplace_status, 'novo')
                                    ELSE mktplace_status
                                  END,
         mktplace_entered_at    = CASE
                                    WHEN p_has_mktplace AND p_mktplace_id IS NOT NULL
                                         AND mktplace_status IS NULL
                                    THEN now()
                                    ELSE mktplace_entered_at
                                  END,
         growth_onboarding_step = 'equipe_designada',
         updated_at             = now()
   WHERE id = p_client_id;

  -- ── Create tag "Esperar Briefing" ──
  INSERT INTO public.client_tags (client_id, name, source)
  VALUES (p_client_id, 'Esperar Briefing', 'growth:equipe_designada')
  ...
  RETURN jsonb_build_object(
    ...
    'assigned', jsonb_build_object(
      'ads_manager', p_ads_manager_id,
      'comercial', p_comercial_id,
      'mktplace', CASE WHEN p_has_mktplace THEN p_mktplace_id ELSE NULL END
    )
  );
END;
$$;

REVOKE ALL ON FUNCTION public.growth_assign_team(uuid, uuid, uuid, uuid, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.growth_assign_team(uuid, uuid, uuid, uuid, boolean) FROM anon;
GRANT EXECUTE ON FUNCTION public.growth_assign_team(uuid, uuid, uuid, uuid, boolean) TO authenticated;

-- =============================================================================
-- 3. MODIFY: create_client_with_automations
--    Add Growth-without-ads path: skip onboarding artifacts, set growth step,
--    create BLOQUEADO tag.
-- =============================================================================
...
COMMENT ON FUNCTION public.create_client_with_automations(jsonb, text) IS
  'Atomic client creation RPC. Roles: executive, gestor_projetos, financeiro, sucesso_cliente. Growth-without-ads path: sets growth_onboarding_step=novos_clientes, creates BLOQUEADO tag, skips ADS onboarding. Error codes: P0001 generic, P0002 invalid payload, P0003 unauthorized, P0004 cnpj duplicate, P0005 other unique, P0006 fk violation.';
...
CREATE OR REPLACE FUNCTION public.trg_dismiss_bloqueado_on_cx_validation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Only fire on transition TO 'validado'
  IF NEW.cx_validation_status = 'validado'
     AND (OLD.cx_validation_status IS DISTINCT FROM 'validado') THEN
    UPDATE public.client_tags
       SET dismissed_at = now(),
           dismissed_by = NULL,
           dismissed_reason = 'cx_validation_completed'
...
DROP TRIGGER IF EXISTS trg_dismiss_bloqueado_on_cx_validation ON public.clients;
CREATE TRIGGER trg_dismiss_bloqueado_on_cx_validation
  AFTER UPDATE OF cx_validation_status ON public.clients
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_dismiss_bloqueado_on_cx_validation();

-- =============================================================================
-- 5. TRIGGER: dismiss "Esperar Briefing" when GP completes "Alinhar Projeto"
--    department_tasks with title ILIKE 'Alinhar Projeto%' transitioning to 'done'
-- =============================================================================
...
BEGIN
  ...
  IF NEW.status = 'done'
     ...
     AND NEW.title ILIKE 'Alinhar Projeto%'
...
DROP TRIGGER IF EXISTS trg_dismiss_esperar_briefing_on_alinhar_done ON public.department_tasks;
CREATE TRIGGER trg_dismiss_esperar_briefing_on_alinhar_done
  AFTER UPDATE OF status ON public.department_tasks
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_dismiss_esperar_briefing_on_alinhar_done();
