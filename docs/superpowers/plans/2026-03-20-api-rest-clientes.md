# API REST para Cadastro de Clientes — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a Supabase Edge Function (`api-v1`) that exposes a REST API for external CRM systems to programmatically create clients, replicating the logic from `useClientRegistration.ts`.

**Architecture:** Single Deno Edge Function with query-param routing (`?action=health|create_client|search_client`). Custom API key auth via `api_keys` table (SHA-256 hashed). Rate limiting via `api_logs` table. All DB operations use `SUPABASE_SERVICE_ROLE_KEY` to bypass RLS.

**Tech Stack:** Deno (Supabase Edge Functions), `@supabase/supabase-js@2.49.2`, Web Crypto API (SHA-256), PostgreSQL

**Spec:** `docs/superpowers/specs/2026-03-20-api-rest-clientes-design.md`
...
|------|--------|---------------|
| `supabase/migrations/20260320100000_create_api_keys_logs_cnpj_unique.sql` | Create | Tables `api_keys`, `api_logs`, CNPJ unique index, seed test key |
| `supabase/functions/api-v1/index.ts` | Create | Edge Function: routing, auth, validation, client creation, logging |
| `supabase/config.toml` | Modify | Add `[functions.api-v1]` with `verify_jwt = false` |
...
**Files:**
- Create: `supabase/migrations/20260320100000_create_api_keys_logs_cnpj_unique.sql`
...
Run: `supabase db push` (or `supabase migration up` if using local dev)
Expected: Tables `api_keys` and `api_logs` created, CNPJ index created, test key seeded.

- [ ] **Step 3: Verify migration**

Run: `supabase db push` again (idempotent check)
Expected: No errors, no changes applied.

- [ ] **Step 4: Commit**

```bash
git add supabase/migrations/20260320100000_create_api_keys_logs_cnpj_unique.sql
git commit -m "feat: add api_keys, api_logs tables and CNPJ unique index for REST API"
```
...
- [ ] **Step 1: Add api-v1 function config**
...
This disables Supabase gateway JWT validation (the function handles its own API key auth).

- [ ] **Step 2: Commit**

```bash
git add supabase/config.toml
git commit -m "feat: add api-v1 edge function config with verify_jwt=false"
```

---

## Task 3: Edge Function — Scaffold, Auth & Health

**Files:**
- Create: `supabase/functions/api-v1/index.ts`

- [ ] **Step 1: Create the Edge Function with routing, API key auth, and health endpoint**
...
Run: `ls -la supabase/functions/api-v1/index.ts`
Expected: File exists.

- [ ] **Step 3: Commit**

```bash
git add supabase/functions/api-v1/index.ts
git commit -m "feat: add api-v1 edge function with health, search, create client endpoints"
```
...
curl -s -H "Authorization: Bearer ***REDACTED***" \
  "https://semhnpwxptfgqxhkoqsk.supabase.co/functions/v1/api-v1?action=health" | jq .
```
...
- [ ] **Step 4: Test search (no results)**

```bash
curl -s -H "Authorization: Bearer ***REDACTED***" \
  "https://semhnpwxptfgqxhkoqsk.supabase.co/functions/v1/api-v1?action=search_client&cnpj=00.000.000/0000-00" | jq .
```

Expected: `{ "found": false }`

- [ ] **Step 5: Test create client (validation error)**
...
curl -s -X POST \
  ...
  -d '{"nome_cliente": ""}' \
  "https://semhnpwxptfgqxhkoqsk.supabase.co/functions/v1/api-v1?action=create_client" | jq .
```

Expected: HTTP 400 with `"code": "VALIDATION_ERROR"` and `details` object listing all missing fields.

- [ ] **Step 6: Test create client (success)**

Use a valid CNPJ for testing (e.g., `11.222.333/0001-81` — valid checksum).
...
curl -s -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "nome_cliente": "Cliente Teste API",
    "razao_social": "Cliente Teste API LTDA",
    "cnpj": "11.222.333/0001-81",
    "nicho": "Tecnologia",
    "observacoes_gestor": "Cliente criado via API para teste",
    "investimento_previsto": 5000,
    "comissao_vendas_percent": 10,
    "data_entrada": "2026-03-20",
    "duracao_contrato_meses": 12,
    "dia_vencimento": 10,
    "produtos_contratados": ["torque-crm"],
    "valores_produtos": {"torque-crm": 2000}
  }' \
  "https://semhnpwxptfgqxhkoqsk.supabase.co/functions/v1/api-v1?action=create_client" | jq .
```

Expected: HTTP 201 with `success: true`, `cliente_id`, and `produtos_criados: ["torque-crm"]`.

- [ ] **Step 7: Test duplicate CNPJ**

Run the same curl command from Step 6 again.
Expected: HTTP 409 with `"code": "DUPLICATE"` and the existing `cliente_id`.

- [ ] **Step 8: Test search (with result)**

```bash
curl -s -H "Authorization: Bearer ***REDACTED***" \
  "https://semhnpwxptfgqxhkoqsk.supabase.co/functions/v1/api-v1?action=search_client&cnpj=11.222.333/0001-81" | jq .
```

Expected: `{ "found": true, "cliente_id": "...", "nome_cliente": "Cliente Teste API", ... }`

- [ ] **Step 9: Verify in Supabase dashboard**

Check in the Supabase dashboard that:
1. Client was created in `clients` table with correct data
2. `client_product_values` has a row for `torque-crm`
3. `financeiro_client_onboarding` has a row for `torque-crm`
4. `financeiro_active_clients` has a row for `torque-crm` with `monthly_value=0`
5. `financeiro_tasks` has a pending task
6. `api_logs` has entries for all requests made
7. CPF field is masked in `api_logs.request_body`

- [ ] **Step 10: Clean up test data**

Delete the test client from the Supabase dashboard (cascading deletes will clean up related records).

- [ ] **Step 11: Final commit (if any fixes were needed)**

```bash
git add -A
git commit -m "fix: adjustments from manual API testing"
```
